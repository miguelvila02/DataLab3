...
def balanced_weights():
    return [
        {"possession": 50.0, "shots_on_target": 4, "corners": 5, "fouls": 11}
        for _ in range(5)
    ]
...
class TestPredictWithCustomWeights:
    ...
    def test_equal_weights(self, sample_home_games, sample_away_games):
        ...
        result_simple = predict_with_custom_weights(
            sample_home_games,
            sample_away_games
        )
...
class TestEdgeCases:
    ...
    def test_extreme_dominance_home(self):
        ...
        dominant_home = [
            {"possession": 70.0, "shots_on_target": 10, "corners": 12, "fouls": 5}
            for _ in range(5)
        ]
        weak_away = [
            {"possession": 30.0, "shots_on_target": 1, "corners": 2, "fouls": 18}
            for _ in range(5)
        ]
...
@pytest.mark.parametrize("possession,expected_range", [
    (70.0, (60.0, 80.0)),
    (50.0, (40.0, 60.0)),
    (30.0, (20.0, 40.0)),
])
...
@pytest.mark.parametrize("shots_on_target,expected_min", [
    (10, 8),
    (5, 4),
    (1, 0.5),
])
